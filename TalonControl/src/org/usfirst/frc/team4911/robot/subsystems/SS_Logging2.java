...
public class SS_Logging2 extends Subsystem {
...
//		return System.currentTimeMillis()/1000.0;
	...
	String[] headers = {
			...
			"delta time",         // 1
			"index",              // 2
			"shooterLift_BusVoltage",
			"shooterLift_ClosedLoopError",
			"shooterLift_CloseLoopRampRate",
			"shooterLift_EncPosition",
			"shooterLift_EncVelocity",
			"shooterLift_DeviceID",
			"shooterLift_Error",
			"shooterLift_FaultForLim",
			"shooterLift_FaultForSoftLim",
			"shooterLift_FaultHardwareFailure",
			"shooterLift_FaultOverTemp",
			"shooterLift_FaultRevLim",
			"shooterLift_FaultUnderVoltage",
			"shooterLift_FirmwareVersion",
			"shooterLift_OutputCurrent",
			"shooterLift_OutputVoltage",
			"shooterLift_Position",
			"shooterLift_Setpoint",
			"shooterLift_Speed",
			"shooterLift_Temperature",
			"shooterLift_ControlMode",
			"shooterLift_isAlive",
			"shooterLift_isControlEnabled",
			"shooterLift_isEnabled", // 26
			};
...
//	final int HEADERBUFFER = 2000;
	final int MINFILENUM = 10;
	final int MAXFILENUM = 30;
	final String EXTENSION = ".csv";
//	final int MAXFILESIZE = 5000000;
//	final String CRLF = "\r\n";
//	byte[] byteBuffer = new byte[HEADERBUFFER];
    ...
    void systemLogger(){
    	...
    	//logKeyOutput(KEYINDEX27,"");
    	
    	lastTime = currentTime;
    }

    public void logOutputTerminateJunk(){
        if (raf != null) {
            try {
            	raf.close();
	        } catch (IOException x) {
    ...
    public void logKeyOutputDoubleJunk(int index, double num, int trim) {
