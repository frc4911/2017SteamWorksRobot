...
//    	DriveMotorFrontLeft.setFeedbackDevice(CANTalon.FeedbackDevice.QuadEncoder);
//    	DriveMotorFrontRight.setFeedbackDevice(CANTalon.FeedbackDevice.QuadEncoder);
    }
    ...
    public void enterMPMode() {
    	DriveMotorFrontLeft.configEncoderCodesPerRev(360); // this must be done, not sure if it goes here or in MotionProfiler
    	DriveMotorFrontRight.configEncoderCodesPerRev(360);
    	
    	//must set encoder type
    	DriveMotorFrontLeft.setFeedbackDevice(CANTalon.FeedbackDevice.QuadEncoder);// this is default, so not needed
    	DriveMotorFrontRight.setFeedbackDevice(CANTalon.FeedbackDevice.QuadEncoder);// this is default, so not needed

    	// also a positive .set to the talon must make the encoder count up
    	//DriveMotorFrontLeft.reverseSensor(true); //not needed for drive motors
    	//DriveMotorFrontRight.reverseSensor(true); //not needed for drive motors
...
//    	driveByMPSub(DriveMotorFrontLeft);
//    	driveByMPSub(DriveMotorFrontRight);
//    	DriveMotorFrontLeft.getMotionProfileStatus(mps);
//    	displayMotionProfileStatus(mps);
    }
    
    void driveByMPSub(CANTalon talon) {
    	SmartDashboard.putNumber("drive left encoder", DriveMotorFrontLeft.getEncPosition());
		SmartDashboard.putNumber("drive right encoder", DriveMotorFrontRight.getEncPosition());
    }
